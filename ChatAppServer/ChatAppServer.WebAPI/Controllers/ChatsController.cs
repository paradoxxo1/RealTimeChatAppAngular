...
[ApiController]
public sealed class ChatsController(ApplicationDbContext context, IHubContext<ChatHub> hubcontex) : ControllerBase
{
    [HttpGet]
    ...
    public async Task<IActionResult> SendMessage(SendMessageDto request, CancellationToken cancellationToken)
    {
        ...
        await hubcontex.Clients.Client(connectionId).SendAsync("Messages", chat);
